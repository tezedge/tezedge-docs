    ...
    return (
      <StaticQuery
        ...
        render={data => {
            ...
            if (root.children) {
              return root.children.map(item => {
                ...
                return (
                  <SubMenu 
                    key={item.key}
                    title={<span style={{fontWeight:400}}
                  >{item.title}</span>}>
                    {loop(item)}
                  </SubMenu>
                )
          ...
          // open all submenus
          defaultOpenKeys = entries.map(item => item.id)
          // add external links
          defaultOpenKeys = [
            ...defaultOpenKeys,
            'further_reading'
          ...
          return (
            ...
            >
              {loop(tree)}
              <SubMenu key="further_reading" title='Futher reading' >
                <Menu.Item key='Medium'>
                  <a href="http://medium.com/simplestaking" target="_blank" rel="noopener noreferrer">Medium</a>
                </Menu.Item>
                <Menu.Item key='Twitter'>
                  <a href="http://twitter.com/simplestaking" target="_blank" rel="noopener noreferrer">Twitter</a>
                </Menu.Item>
                <Menu.Item key='Github'>
                  <a href="http://github.com/simplestaking/tezedge" target="_blank" rel="noopener noreferrer">Github</a>
                </Menu.Item>
              </SubMenu>
            </Menu>
          )
